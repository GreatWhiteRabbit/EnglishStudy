...
<div>
  ...
  <div class="content">
    <el-row class="container-marked">
      <el-col class="blog-content" :span="22" :offset="1">
        <div>
          ...
          <el-col>
            ...
            <el-row style="margin-bottom: 5px">
              <el-button
                  v-show="passageEdit"
                  @click="updatePassage" style="margin-left: 100%;font-size: 25px"
        ...
        <div style="font-family:'Hiragino Sans GB';color: #27db27" v-show="addQuestionEdit">
          <div>
            <el-row style="margin-top: 20px;border-top: groove">
              <el-col >
                <span>{{addQuestionForm.title}}</span>
              </el-col>
            </el-row>
            <el-row style="margin-top: 10px">
              <el-col :span="20">
                <el-row>
                  <el-col >
                    {{addQuestionForm.optionsA}}
                  </el-col>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-col >{{addQuestionForm.optionsB}}</el-col>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-col>{{addQuestionForm.optionsC}}</el-col>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-col>{{addQuestionForm.optionsD}}</el-col>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-col>正确答案 ：{{addQuestionForm.answer}}</el-col>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-col>详细解答：{{addQuestionForm.explanation}}</el-col>
                </el-row>
              </el-col>
              <el-col :span="4">
                <el-row>
                  <el-button @click="showAddQuestionDialog" type="primary" icon="el-icon-edit" circle></el-button>
                </el-row>
                <el-row>
                  <el-button @click="addQuestion()"
                             v-show="addQuestionEdit === true "
                             type="primary" icon="el-icon-document-checked" circle></el-button>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-button @click="cancelAddQuestion()" type="danger" icon="el-icon-delete" circle></el-button>
                </el-row>
              </el-col>
            </el-row>
          </div>
        ...
        <div style="font-family:'Hiragino Sans GB';color: #767778">
          <div v-for="(item,index) in questionList" :key="index" style="">
            ...
            <el-row style="margin-top: 10px">
              ...
              <el-row style="margin-top: 10px">
                <el-col >{{item.optionsB}}</el-col>
              </el-row>
              <el-row style="margin-top: 10px">
                <el-col>{{item.optionsC}}</el-col>
              </el-row>
              <el-row style="margin-top: 10px">
                <el-col>{{item.optionsD}}</el-col>
              </el-row>
              <el-row style="margin-top: 10px">
                <el-col>正确答案 ：{{answerList[index].answer}}</el-col>
              </el-row>
              <el-row style="margin-top: 10px">
                <el-col>详细解答：{{answerList[index].explanation}}</el-col>
              </el-row>
              </el-col>
              <el-col :span="4">
                <el-row>
  ...
  <el-dialog title="修改阅读理解" :visible.sync="passageDialogShow">
    <el-form :model="passageForm">
      ...
      <el-form-item label="每日英语内容" label-width="100px" >
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="updatePassage">更 新</el-button>
  ...
  <el-dialog title="添加问题及选项" :visible.sync="addQuestionDialogShow">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="addQuestion">更 新</el-button>
...
export default {
  ...
  data() {
    return{
      ...
      questionList:[],
      answerList:[],
      passageDialogShow:false,// 显示修改文章内容或者标题窗口
  ...
  methods:{
      ...
      this.HttpGet('/passage/' + this.passageId)
          .then(res => {
            if(res.success === false) {
              this.$router.push('/404'); // 没有获取到内容跳转到404
            }
            else {
              this.passage.title = res.data.title;
          ...
          .then(res => {
            ...
            for(let i = 0; i < this.answerList.length; i++) {
              for(let j = 0; j < this.questionList.length; j++) {
                if(this.questionList[j].questionId === this.answerList[i].questionId) {
                  this.questionList[j].answer = this.answerList[i].answer;
                  this.questionList[j].explanation = this.answerList[i].explanation;
                }
              }
    ...
    updatePassage() {
      if(this.passageForm.title !== "" && this.passageForm.content !== "") {
        // 发送网络请求
        this.HttpPost('/passage/update',
            {
              "passageId":this.passageId,
              "title":this.passageForm.title,
              "content":this.passageForm.content
            })
            .then(res => {
              if(res.success === true) {
                this.$notify({
    ...
    showPassageDialog() {
      ...
      if(this.passageEdit === false) {
        this.passageForm.title = this.passage.title;
        this.passageForm.content = this.passage.content;
      }
    },
    // 修改选项
    updateQuestion() {
      // 发送网络请求
      let explanation = "略";
      if(this.questionForm.explanation !== "") explanation = this.questionForm.explanation;
      this.HttpPost('/question/update',{
        "questionId": this.questionList[this.questionIndex].questionId,
        "passageId": this.passageId,
        "title": this.questionForm.title,
        "optionsA": this.questionForm.optionsA,
        "optionsB": this.questionForm.optionsB,
        "optionsC": this.questionForm.optionsC,
        "optionsD": this.questionForm.optionsD,
    ...
    showQuestionDialog(index) {
      if(this.questionEdit === true && this.questionIndex !== index) {
        // 表示还有修改没有保存
        this.$notify({
          message:"还有修改未保存",
          type:'warning'
    ...
    deleteQuestion(index) {
      this.HttpPost("/question/" + this.questionList[index].questionId)
          .then(res => {
            if(res.success === true) {
              this.getPassage();
              this.$notify({
    ...
    addQuestion() {
       ...
       data.optionsD = this.addQuestionForm.optionsD;
        ...
        data.explanation = this.addQuestionForm.explanation === "" ? "略" : this.addQuestionForm.explanation;
        questionList.push(data);
        if(data.title === "" || data.optionsA === "" || data.optionsB === ""
        || data.optionsC === "" || data.optionsD === "" || data.answer === "") {
          this.$notify({
            message:"内容不能为空",
            type:'warning'
          })
    ...
    // 预览passage
    previewPassage(){
      this.passage.title = this.passageForm.title;
      this.passage.content = this.passageForm.content;
      this.passageDialogShow = false;
  ...
  mounted() {
    ...
    window.onbeforeunload = function(e) {
      ...
      if (_this.questionEdit === true || _this.addQuestionEdit === true || _this.passageEdit === true) {
        if (_this.$route.path == "/updatePassage/" + _this.passageId) {
