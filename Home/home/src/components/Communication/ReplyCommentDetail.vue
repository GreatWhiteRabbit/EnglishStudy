  ...
  <div class="comment">
    <el-row>
      <el-row style="margin-bottom: 25px;">
        <el-col :span="1">
          <el-avatar :size="60" :src="firstLeverComment.image"></el-avatar>
        </el-col>
        <el-col :span="20" style="margin-left: 15px;margin-top: 10px; border-bottom: 0.005em solid #e0dfdf">
          <span style="color: rgb(66, 66, 66)">{{firstLeverComment.nickName}}</span>
          <span style="margin-left: 20px;color: gray;">{{getTime(firstLeverComment.time)}}</span>
          <el-tag style="margin-left: 10px;font-size: 18px"
                  v-show="firstLeverComment.status === 1"
                  type="danger" effect="dark"> 管理员</el-tag>

          <el-tag style="font-size: 18px; margin-left: 10px;"
                  type="success" key="置顶" effect="dark"
                  v-show="firstLeverComment.top === true">置顶</el-tag>
          <span style="float: right;margin-right: 5%;
          color: gray;
          user-select: none; -webkit-user-select: none;
          -moz-user-select: none;-ms-user-select:none;"
          @click="replyComment(11)"
          >回复</span>
          <!--        下面是回复功能-->
          <el-row style="margin-top: 15px">
            <el-row>
              <el-col>
                <span style="font-size: 20px;color: #585d5d" v-html="analyzeContent(firstLeverComment.content)"></span>
              </el-col>
              <el-col style="margin-top: 20px;" v-show="inputIndex === 11">
            ...
            <el-row>
              ...
              <el-row style="background-color:  #fafafa;border-radius: 5%">
                <!-- 二级评论             -->
                <el-row style="margin-bottom: 25px;"
                        v-for="(secondItem,childIndex)  in secondLeverComment"
                        :key="childIndex">
                  <el-col :span="1">
                  ...
                  <el-col :span="20" style="margin-left: 15px;margin-top: 10px;">
                    ...
                    <el-button  style="float: right;margin-right: 30%"
                                ...
                                @click="deleteComment(childIndex)"
                                circle></el-button>
                    ...
                    <el-row style="margin-top: 15px">
                      <el-row>
                        ...
                        <el-col style="margin-top: 20px;" >
                          <el-row class="comment" v-show="inputIndex === childIndex">
                            <el-row>
                              <el-input
                                  type="textarea"
                            ...
                            <el-row>
                              ...
                              <el-col :span="12">
                                <el-button
                                    ...
                                    round
                                    style="font-size: 18px"
                                    @click="submitComment(childIndex)"
                                >发布</el-button>
                              </el-col>
                            </el-row>
                          </el-row>
...
export default {
  ...
  data() {
    return {
      userId: '',
      replyCommentId: '',
      firstLeverComment:'', // 一级评论的相关内容
      secondLeverComment:[], // 二级评论数组
      inputIndex:-1,
      total:0,
      currentPage:1,
      currentSize:10,
  ...
  methods: {
   // 获取一级评论
    getFirstLeverComment() {
      this.HttpGet('/comment/reply/comment/first/' + this.replyCommentId)
          .then(res => {
            if(res.data.commentId === 0) {
              this.$notify({
                message:'原顶级评论已被删除，不能查看顶级评论下面的留言信息，即将跳转到404页面',
                title:'评论提示',
                type:'error',
                onClose() {
              ...
              this.firstLeverComment = res.data;
              // 获取二级评论
              this.getSecondLeverComment();
            }
          })
    },
    // 获取二级评论
    getSecondLeverComment() {
      this.HttpGet('/comment/second/' + this.replyCommentId + "/" + this.currentPage + "/" + this.currentSize)
          .then(res => {
            this.total = res.data.total;
            this.secondLeverComment = res.data.secondCommentList;
          })
    },

    // 删除评论
    deleteComment(index) {
      // index = 11代表一级评论且不能删除一级评论
      let userId,commentId;
      userId = this.secondLeverComment[index].userId;
      commentId = this.secondLeverComment[index].commentId;
      this.HttpPost('/comment/delete/' + userId + '/' + commentId)
          .then(res => {
            if(res.data === true) {
              this.$message.success("删除成功");
              this.secondLeverComment.splice(index,1);
            }
            else {
    ...
    submitComment(index) {
      ...
      if(submit === true) {
        ...
        // 回复一级评论
        if(index === 11) {
          data.userId = this.firstLeverComment.userId;
          data.replyCommentId = this.firstLeverComment.commentId;
        }
        else {
          data.userId = this.secondLeverComment[index].userId;
          data.replyCommentId = this.secondLeverComment[index].replyCommentId;
        }
  ...
  created() {
      ...
      this.replyCommentId = this.$route.query.replyCommentId;
      this.getFirstLeverComment();
  },
  computed:{
    // 将时间戳转换为几小时前
    getTime() {
