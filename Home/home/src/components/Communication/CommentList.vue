...
<div>
  <el-row>
    <el-row style="margin-bottom: 25px;" v-for="(item,index)  in list" :key="index">
      <el-col :span="1">
        <el-avatar :size="60" :src="item.image"></el-avatar>
      </el-col>
      <el-col :span="20" style="margin-left: 15px;margin-top: 10px; border-bottom: 0.005em solid #e0dfdf">
            <span style="color: rgb(66, 66, 66)">{{item.nickName}}</span>
            <span style="margin-left: 20px;color: gray;">{{getTime(item.time)}}</span>
            <el-tag style="margin-left: 10px;font-size: 18px"
                    v-show="item.status === 1"
                    type="danger" effect="dark"> 管理员</el-tag>

            <el-tag style="font-size: 18px; margin-left: 10px;"
                type="success" key="置顶" effect="dark"
                  v-show="item.top === true">置顶</el-tag>
            <el-button style="margin-left: 10px"
                       v-show="getCurrentUser() === 1 "
                       type="primary" size="small"
                       @click="setTopOrNot(index)"
                       round>{{getTop(item.top)}}</el-button>

        <el-button v-show="getCurrentUser() === item.userId || getCurrentUser() === 1"
                   style="float: right;margin-right: 30%;"
                   type="danger" icon="el-icon-delete"
                   @click="deleteComment(index,-1)"
                   circle></el-button>
...
<!--        下面是回复功能-->
        <el-row style="margin-top: 15px">
          <el-row>
            <el-col>
              <span style="font-size: 20px;color: #585d5d" v-html="analyzeContent(item.content)"></span>
            </el-col>
            <el-col style="margin-top: 20px;" v-show="firstIndex === index && secondIndex === -1">
              <el-row class="comment">
                <el-row>
                  <el-input
                      type="textarea"
                      placeholder="快点留下的你足迹证明你来过吧꒰ঌ( ⌯' '⌯)໒꒱"
                      :autosize="{ minRows:3 }"
                ...
                <el-row style="margin-left: 50%">
                  <span  style="font-size: 20px;color: #f56c6c">{{ getContentNotify(maxLength-contentLength) }}</span>
                </el-row>
                <el-row>
                  <el-col :span="12">
                    <el-col :span="4">
          ...
          <el-row>
            ...
            <el-row style="background-color:  #fafafa;border-radius: 5%">
<!-- 二级评论             -->
              <el-row style="margin-bottom: 25px;"
                      v-for="(secondItem,childIndex)  in list[index].secondCommentPage.secondCommentList"
                      :key="secondIndex">
                <el-col :span="1">
                  <el-avatar :size="40" :src="secondItem.image"></el-avatar>
                </el-col>
                <el-col :span="20" style="margin-left: 15px;margin-top: 10px;">
                  <span style="color: rgb(66, 66, 66)">{{secondItem.nickName}}</span>
                  <span style="margin-left: 20px;color: gray;">{{getTime(secondItem.time)}}</span>
                  <el-tag style="margin-left: 10px;font-size: 18px"
                          v-show="secondItem.status === 1"
                          type="danger" effect="dark"> 管理员</el-tag>

<!--                删除按钮  -->
                  <el-button  style="float: right;margin-right: 30%"
                              v-show="getCurrentUser() === secondItem.userId || getCurrentUser() === 1"
                             type="danger" size="small" icon="el-icon-delete"
                              @click="deleteComment(index,childIndex)"
                              circle></el-button>
          ...
          user-select: none; -webkit-user-select: none;
          -moz-user-select: none;-ms-user-select:none;"
                        @click="reply(index,childIndex)"
                  >回复</span>
                  <el-row style="margin-top: 15px">
                    <el-row>
                      <el-col>
                        <span style="color: #12abdc;font-size: 17px;margin-right: 5px">@{{secondItem.replyNickName}}</span>
                        <span style="font-size: 20px;color: #585d5d" v-html="analyzeContent(secondItem.content)"></span>
                      </el-col>
                      <el-col style="margin-top: 20px;" v-show="firstIndex === index && childIndex === secondIndex">
                        <el-row class="comment">
                          <el-row>
                            <el-input
                                type="textarea"
                                placeholder="想想怎么回复吧°꒰๑'ꀾ'๑꒱°"
                                :autosize="{ minRows:3 }"
                                style="width: 60%;font-size: 20px;"
                                @input="getContentLength"
                                v-model="inputContent"
              ...
              <el-row v-show="item.secondCommentPage.total > 10">
              <el-pagination

                  layout="sizes, prev, pager, next"
                  :page-size="secondPages[index].currentSize"
                  :current-page.sync="secondPages[index].currentPage"
                  @current-change="((val)=>{handleCurrentChange(val, index)})"
                  page-sizes="10"
                  :total="item.secondCommentPage.total"
                  style="display: flex;justify-content: center;margin-bottom:10px;margin-right: 30%;"
              >
              </el-pagination>
              </el-row>
            </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      firstIndex:-1,// 一级评论的相关按钮
      secondIndex:-1, // 二级评论的相关按钮
      inputContent:'', // 内容
      ...
      emotion:[
        ...
        { title: "发红包", url: "https://localhost:7031/resouce/image/3385eece-ea09-432b-837e-8955f6ce1415.gif" },
        { title: "抱拳", url: "https://localhost:7031/resouce/image/d31f18e1-a06d-40f0-9872-cb0d836f331d.png" },
      ],
  ...
  methods:{
    // 设置置顶或者取消置顶
    setTopOrNot(index) {
        let commentId = this.list[index].commentId;
        let top = this.list[index].top;
        this.HttpPost('/comment/top/' + commentId + "/" + top)
            .then(res => {
              if(res.data === true) {
    ...
    // 删除评论
    deleteComment(firstIndex,secondIndex) {
      let userId,commentId;
      // 删除一级评论
      if(secondIndex === -1) {
        userId = this.list[firstIndex].userId;
        commentId = this.list[firstIndex].commentId;
      }
      // 删除二级评论
      else {
          userId = this.list[firstIndex].secondCommentPage.secondCommentList[secondIndex].userId;
          commentId = this.list[firstIndex].secondCommentPage.secondCommentList[secondIndex].commentId;
      }
      this.HttpPost('/comment/delete/' + userId + '/' + commentId)
          .then(res => {
            if(res.data === true) {
    ...
    submitComment() {
      ...
      if(submit === true) {
        ...
        if(this.secondIndex !== -1) {
          data = this.list[this.firstIndex].secondCommentPage.secondCommentList[this.secondIndex];
        }
        else {
          data = this.list[this.firstIndex];
          data.replyCommentId = data.commentId;
        }
          ...
          content: this.inputContent
            ...
            .then(res => {
              if (res.data === 0) {
                ...
                this.inputContent = "";
                this.getSecondCommentList(this.firstIndex);
              }
              this.firstIndex = -1;
              this.secondIndex = -1;
            })
    ...
    handleCurrentChange(val,index) {
      this.secondPages[index].currentPage = val;
      this.getSecondCommentList(index);
    },
    // 分页获取二级评论
    getSecondCommentList(index) {
      let replyCommentId = this.list[index].commentId;
      this.HttpGet('/comment/second/' + replyCommentId + '/' + this.secondPages[index].currentPage + "/" + this.secondPages[index].currentSize)
          .then(res => {
            this.list[index].secondCommentPage.secondCommentList = res.data.secondCommentList;
          })
    },
    ...
    analyzeContent(content) {
      var emotionPattern1 = /\$[\u4e00-\u9fa5]+\:/g;
      var emotionPattern2 = /\$[\u4e00-\u9fa5]+\:/;
      var linkPattern1 = /(?<=#).*(?=;)/g;
      var linkPattern2 = /#(\S*);/;
      var imagePattern1 = /(?<=~).*(?=})/g;
      var imagePattern2 =/~(\S*)}/;
      var html = content;
      var matchEmotion = content.match(emotionPattern1);
      if(matchEmotion) {
        // 匹配表情包
        for (let i = 0; i < matchEmotion.length; i++) {
          var url = "";
          for (let j = 0; j < this.emotion.length; j++) {
            if ("$" + this.emotion[j].title + ":" === matchEmotion[i]) {
              // url = "<img src='" + this.emotion[j].url + "'/>";
              url = '<img src="' + this.emotion[j].url +  '" width="30" height="30"/>'
              break;
            }
          }
          html = html.replace(emotionPattern2,url);
        }
      }
      // 匹配链接
      var matchLink = content.match(linkPattern1);
      if(matchLink) {
        for(let i = 0; i < matchLink.length; i++) {
          let link = matchLink[i].split("***");
          let a = '<a href="' + link[0] + '"' + 'target="' + '_blank"' + '>' + link[1] + '</a>';
          html = html.replace(linkPattern2,a);
        }
      }
      // 匹配图片
      var matchImage= content.match(imagePattern1);
      if(matchImage) {
        for(let i = 0; i < matchImage.length; i++) {
          let img = '<img src="' + matchImage[i] + '" width="150" height="150"/>'
          html = html.replace(imagePattern2,img);
        }
      }
      return html;
    },
  ...
  computed:{
    ...
    getContentNotify() {
      return (length) => {
        if(length >=0) {
          return "还可输入" + length + "字";
        }
        else {
          return "超出" + (0-length) + "个字Σ( ° △ °|||)︴";
        }
      }
    }
  }
