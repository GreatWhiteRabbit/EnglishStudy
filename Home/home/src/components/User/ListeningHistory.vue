...
export default {
  ...
  methods:{
    // 获取听力做题记录
    getListeningRecordList() {
      let userId = localStorage.getItem("userId");
      this.HttpGet('/paperRecord/page/' + userId + '/' + this.currentPage + "/" + this.currentSize)
          .then(res => {
    ...
    getAccuracy() {
      let userId = localStorage.getItem("userId");
      this.HttpGet('/paperRecord/accuracylist/' + userId + '/' + 10)
          .then(res => {
    ...
    deleteRecordById(index) {
      this.$confirm('确定删除此记录', '提示', {
        ...
        let userId = localStorage.getItem("userId");
        let list = [];
        list.push(this.record[index].listeningPaperRecordId);
    ...
    // 前往详细页
    goToDetail(index) {
      let id = this.record[index].listeningPaperId;
