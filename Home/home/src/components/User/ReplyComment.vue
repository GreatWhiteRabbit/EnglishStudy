<template>
  <div>
    <el-row style="margin-bottom: 10px;color: #7c7878;font-size: 20px;">
      回复我的
      <el-button style="margin-left: 70%;background-color: #3cdee0;color: white;font-size: 18px"
                 size="small" @click="readAllReply" round>全部已读
      </el-button>
    </el-row>
    <el-row v-show="message.length === 0">
      <el-empty description="暂无数据"></el-empty>
    </el-row>
    <el-row v-for="(item,index) in message" :key="item.commentReceiveId">
      <el-row>
        <el-col @click.native="goToComment(index)" :span="20">
          <el-row>
            <el-col :span="2">
              <el-avatar :size="40" :src="item.image"></el-avatar>
            </el-col>
            <el-col :span="20">
              <el-row>
              ...
              <el-row>
                <span style="font-size: 20px;color: gray" v-html="analyzeContent(item.content)"></span>
              </el-row>
            </el-col>
          </el-row>
        ...
        <el-col :span="2">
          <el-button style="background-color: wheat;font-size: 18px"
                     @click="replyComment(index)" round>回复
          </el-button>
        </el-col>
      </el-row>
      <el-row v-show="inputIndex === index">
          <el-row>
            <el-input
                type="textarea"
                placeholder="要认真回复别人哦"
                :autosize="{ minRows:3 }"
              ...
              <el-col :span="5">
                ...
                <el-card v-show="showEmotion" class="box-card">
                  <div slot="header" class="clearfix">
                    <span>表情</span>
                    <el-button @click="showEmotion = false" style="float: right; padding: 3px 0" type="text">X
                    </el-button>
                  </div>
    ...
    <el-row v-show="message.length !== 0">
      ...
      <span @click="nextPageReplyMessage" style="
...
export default {
  ...
  data() {
    return {
      ...
      emotion: [
        ...
        {
          title: "发红包",
          url: "https://localhost:7031/resouce/image/3385eece-ea09-432b-837e-8955f6ce1415.gif"
  ...
  methods: {
    ...
    submitComment() {
      ...
      if(submit === true) {
        ...
        this.HttpPost('/comment/add',{
          replyCommentId:this.message[this.inputIndex].replyCommentId,
          userId:userId,
          replyUserId:this.message[this.inputIndex].userId,
    ...
    getReplyMessage() {
      let userId = localStorage.getItem("userId");
      this.HttpGet('/comment/detail/page/' + userId + '/' + this.currentPage + '/' + this.currentSize)
          .then(res => {
            if (res.data.replyComments.length === 0) {
              this.noMoreMessage = true;
            } else {
              for (let i = 0; i < res.data.replyComments.length; i++) {
                this.message.push(res.data.replyComments[i]);
              }
            }
          })
    },
    ...
    readAllReply() {
      let userId = localStorage.getItem("userId");
      this.HttpPost('/comment/read/all/' + userId)
          .then(res => {
            if (res.data === true) {
              for (let i = 0; i < this.message.length; i++) {
                this.message[i].read = 1;
              }
            }
          })
    },
    ...
    // 获取更多回复消息
    nextPageReplyMessage() {
      this.currentPage += 1;
      this.getReplyMessage();
    },
    // 跳转到评论区
    goToComment(index) {
      let replyCommentId = this.message[index].replyCommentId;
      let userId = localStorage.getItem("userId");
      this.$router.push({
    ...
    analyzeContent(content) {
      ...
      if (matchImage) {
        for (let i = 0; i < matchImage.length; i++) {
          let img = '<img src="' + matchImage[i] + '" width="100" height="100"/>'
