...
import { Loading,Notification} from 'element-ui'    // 这里我是使用elementUI的组件来给提示
...
const instance = axios.create({    //创建axios实例，在这里可以设置请求的默认配置
    ...
    baseURL: process.env.NODE_ENV === 'production' ? '/apis' : '/apis'   //根据自己配置的反向代理去设置不同环境的baeUrl
})
...
instance.interceptors.request.use(config => {
    ...
    let tokenPrefix = "Bearer" + " "; // .net的JWTBearer需要在token前面加上这些才能自动获取token
    let token = localStorage.getItem("token") || '';
    config.headers['Authorization'] = tokenPrefix + token;
    let loading = true; // 加载动画
    if(config.url.includes('resource/download')) {
        loading = false; // 不加载动画
    }
    if(loading){
    ...
    if (config.url.includes('resource/download')) {
       ...
       // config.headers['responseType'] = 'blob'; // 这里是错的
    }
